...
def build_initial_prompt(brief: CreativeBrief, *, has_menu_images: bool = False, has_logo_images: bool = False, has_reference_images: bool = False) -> str:
    ...
    if has_logo_images:
        lines.extend(
            [
                "로고 이미지: 첨부된 로고를 최종 홍보물 안에 반드시 넣는다.",
                "로고의 글자, 심볼, 비율, 색감을 임의로 다시 디자인하거나 다른 로고처럼 바꾸지 않는다.",
                "로고는 배경과 분리되어 선명하게 읽히도록 상단 또는 하단의 여백에 배치한다.",
            ]
        )
    if has_reference_images:
        lines.append("참고 디자인 이미지: 색감, 여백, 정보 위계, 구도만 참고하고 음식 사진이나 로고보다 우선하지 않는다.")
    lines.extend(
        [
            "첨부된 메뉴 사진 또는 로고가 있는 경우 최종 결과에서 누락하면 실패한 결과다.",
